...
controller.hears(["espacio"], 'direct_message,direct_mention', function (bot, message) {
            ...
            bot.startConversation(message, function (err, convo) {
                var question = "Qué nombre quieres darle al nuevo espacio?";
                convo.ask(question, [
                  {
                        default: true,
                        ...
                        async.waterfall([
                               function (callback){
                                  spark.rooms.create({"title": roomName}, function(err, response) {
                                      if(err)
                                      {
                                          convo.say("Lo siento, ha habido un error creando el espacio, por favor contacta con el administrador.");
                                          convo.next(); 
                                        }
                                      else{
                              ...
                              function (response, roomId, callback){
                                 ...
                                 if(responseObj.items.length > 0){
                                   responseObj.items.forEach(function(element) {
                                    ...
                                    convo.say("Espacio creado, ya puedes comprobarlo.");
                                    convo.next();
                                 }
                                 else{
                                    convo.say("Lo siento, ha habido un error creando el espacio, por favor contacta con el administrador.");
